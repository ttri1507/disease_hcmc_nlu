...
parser = argparse.ArgumentParser(description='Disease prediction script for Dengue Fever')
...
combined_file = os.path.join(output_folder, 'combined_data_dengue.csv')  # File mới kết hợp dữ liệu gốc và dự đoán
...
def process_district(district):
    # Filter data 
    tieu_chay_district = df[df['T_BTT'] == 'Sốt xuất huyết Dengue'][['NAM', 'TUAN', district]].copy()
    ...
    plt.title(f'Dự đoán và thực tế số ca Sốt xuất huyết Dengue tại {district} năm 2022-2024')
    ...
    # save fig
    plot_path = os.path.join(output_folder, f'prediction_plot_{district}_dengue.png')
...
predict_df_2024 = pd.DataFrame({
    'ID': range(len(df[df['T_BTT'] == 'Sốt xuất huyết Dengue']), len(df[df['T_BTT'] == 'Sốt xuất huyết Dengue']) + 52),
    'NAM': [2024] * 52,
    'TUAN': range(1, 53),
